...
char *newName(const char *name) 
{
    int act = 0;
    while (name[act++] != '\0');
    char *newName = malloc(act * sizeof(char));
    while (act-- > -1) 
        if (name[act] >= 'A' && name[act] <= 'Z')
            newName[act] = (char)(name[act] - 'A' + 'a') 
        else
            newName[act] = name[act];
    return newName;
}
...
char *newNumber(const char *number) 
{
    ...
    while (number[act++] != '\0') 
        if (number[act] >= '0' && number[act] <= '9')
            newSize++;
    char *newNumber = malloc((newSize + 1) * sizeof(char));
    act = 0;
    while (newSize > 0)
    {
        if (number[act] >= '0' && number[act] <= '9') 
        {
            newNumber[size++] = number[act];
            newSize--;
        }
        act++;
    }
...
{
    int act = 0;
    char *newName = newName(name);
    while (newName[act++] != '\0') 
        if (newName[act] < 'a' || newName[act] > 'z') 
            return 0;
    return 1;
}
...
void find(char *data) 
{
    ...
    int act = 0, found = 0;
    char *newName = newName(data), *newNumber = newNumber(data);
    while (act++ < book.size) 
    {
        if (strstr(newName(book.contacts[act].name), newName) ||
            !strcmp(newNumber(book.contacts[act].number), newNumber)) 
        {
